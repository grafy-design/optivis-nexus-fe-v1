...
export function FormulaCard({
  ...
  return (
    <div
      className={`rounded-[18px] overflow-hidden max-w-[390px] ${className}`}
      style={{
    ...
    >
      <div className="p-4 pb-1 flex flex-col gap-4">
        {/* Title */}
        <div className="text-body4 text-white ">
          {title}
        </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {usedValues.map((item, index) => (
              ...
              >
                ...
                <span className="text-body3 text-[#ebebf0]">
                  {item.value}
                </span>
              </div>
            ))}
              ...
              {definitions.map((def, index) => (
                <div key={index} className="flex gap-1">
                  <div className="flex-shrink-0 w-[20px]">
                    <div className="text-small1 text-[#ebebf0]">
                      <FormulaDisplay formula={`${def.symbol}:`} displayMode={false} />
                    </div>
                  </div>
