...
export function MultiRankingBarChart({ data, height = "100%", label: _label }: MultiRankingBarChartProps) {
  const [isNarrow, setIsNarrow] = useState(false);

  useEffect(() => {
    const update = () => setIsNarrow(window.innerWidth < 1470);
    update();
    ...
    <ReactECharts
      option={{
        ...
        series: normalizedData.map((item, index) => ({
          ...
          data: [item.value],
          itemStyle: {
            color: index < 3 ? "#f06600" : index >= 7 ? "#C8C8CB" : "#AAAAAD",
