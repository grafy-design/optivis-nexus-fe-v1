...
/**
 * DRD 전용 헤더 컴포넌트
 * Figma 디자인을 바탕으로 1(Default Settings), 2(Simulation Settings), 3(Dashboard) 단계를 표시합니다.
 */
export const DrdHeader = ({ step }: DrdHeaderProps) => {
  const router = useRouter();
  const searchParams = useSearchParams();
  const isStep2Disabled = step < 2;
  const isStep3Disabled = step < 3;
  ...
  const resolveTaskId = (): string | null => {
    ...
    const candidate = fromQuery?.trim();
  ...
  return (
    <header className="flex items-center justify-between px-[28px] py-[17px] h-[90px] w-full sticky top-0 z-[90] shrink-0 bg-[#e7e5e7]">
      {/* ── 왼쪽: 단계별 상태 ────────────────────────────────────────────────── */}
      <div className="flex items-center gap-[36px]">
        ...
        <button
          ...
          className={`flex items-center gap-[8px] bg-transparent border-none p-0 ${isStep2Disabled ? "cursor-not-allowed" : "cursor-pointer"}`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-[8px] bg-transparent border-none p-0 ${isStep3Disabled ? "cursor-not-allowed" : "cursor-pointer"}`}
        >
