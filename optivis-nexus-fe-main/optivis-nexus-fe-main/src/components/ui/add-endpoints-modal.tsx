...
function validateThreshold(type: string, threshold: string): string | null {
  if (type !== "Binary") return null;
  const trimmed = threshold.trim();
  if (!trimmed) return "Required for Binary";
...
export default function AddEndpointsModal({
  ...
  const primaryThresholdErrors = useMemo(() => {
    const errors: Record<string, string> = {};
    for (const row of primaryRows) {
      const error = validateThreshold(row.type, row.threshold);
      if (error) errors[row.id] = error;
    }
    return errors;
  }, [primaryRows]);

  const secondaryThresholdErrors = useMemo(() => {
    const errors: Record<string, string> = {};
  ...
  const isFormValid = useMemo(
    () =>
      Object.keys(primaryThresholdErrors).length === 0 &&
      Object.keys(secondaryThresholdErrors).length === 0,
    [primaryThresholdErrors, secondaryThresholdErrors],
  );

  const isSaveActionDisabled = isSaveDisabled || !hasChanges || !isFormValid;
            ...
            <div className="relative z-10 p-6 flex flex-col">
              <div className="flex items-center justify-between mb-14">
                ...
                <div className="flex gap-3 items-center">
                  <button
                    onClick={() => {
                      if (isSaveActionDisabled) return;
                        ...
                        <tr key={row.id}>
                          ...
                          <td className="p-1 align-middle">
                            <div>
                              ...
                              {row.type === "Binary" &&
                              primaryThresholdErrors[row.id] ? (
                                <p className="mt-1 text-[10px] leading-[1.2] text-[#ef4444]">
                                  {primaryThresholdErrors[row.id]}
                                </p>
                              ) : null}
                            </div>
                            ...
                            <div>
                              <input
                                ...
                                className={`w-full h-[26px] px-3 rounded-[8px] bg-neutral-90 text-body5 text-neutral-50 border-0 outline-none ${
                                  ...
                                  row.type === "Binary" &&
                                  secondaryThresholdErrors[row.id]
                                    ? "ring-1 ring-[#ef4444]"
                                    : ""
                                }`}
                              />
