...
export function RefineCutoffChartEditor({
  ...
  const updatePlotArea = () => {
    ...
    try {
      ...
      // Ignore until the chart finishes its first render.
    }
  };
  ...
  useEffect(() => {
    const timer = window.setTimeout(updatePlotArea, 50);
      ...
      xAxis: {
        ...
        axisLabel: {
          ...
          formatter: (value: number) => {
            if (Math.abs(value - xAxisBounds.min) < EPSILON) {
              return `{lEdge|${value.toFixed(0)}}`;
            }
            if (Math.abs(value - xAxisBounds.max) < EPSILON) {
              return `{rEdge|${value.toFixed(0)}}`;
            }
      ...
      <div
        ...
        >
          ...
          >
            <div
              ...
              onMouseDown={(event) => {
                ...
                const handleMouseMove = (moveEvent: MouseEvent) => {
                  ...
                  const percentage = clamp(Math.round(pixelYToProportion(mouseY)), 0, 100);
            ...
            >
              <img
                src="/assets/icons/chevron-select.svg"
                width={12}
                height={12}
                alt=""
                style={{ flexShrink: 0 }}
