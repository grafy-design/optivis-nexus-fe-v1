...
import { getDrdGroupManageFilterInfo, saveDrdGroupManageFilter } from "@/services/drd-service";
    ...
    >
      ...
      <div
        style={{
          ...
          inset: 0,
          borderRadius: 36,
          border: pressed ? "2px solid rgba(51, 0, 255, 0.4)" : "2px solid rgba(255,255,255,0.3)",
          boxShadow: "0px 0px 2px 0px rgba(0,0,0,0.05)",
...
function FilterPageContent() {
  ...
  const [inclusionSections, setInclusionSections] = useState<Section[]>(
    filterData.inclusion.length > 0 ? filterData.inclusion : [makeEmptySection(1)]
  );
  const [exclusionSections, setExclusionSections] = useState<Section[]>(
    filterData.exclusion.length > 0 ? filterData.exclusion : [makeEmptySection(1)]
  );
  // 각 섹션의 아코디언 열림 여부 (sectionId → boolean)
  const [inclusionOpenSections, setInclusionOpenSections] = useState<Record<number, boolean>>(
    filterData.inclusion.length > 0
      ? Object.fromEntries(filterData.inclusion.map((s) => [s.id, true]))
      : { 1: true }
  );
  const [exclusionOpenSections, setExclusionOpenSections] = useState<Record<number, boolean>>(
    filterData.exclusion.length > 0
      ? Object.fromEntries(filterData.exclusion.map((s) => [s.id, true]))
      : { 1: true }
  );
  ...
  const currentOpenSections =
    activeTab === "Inclusion" ? inclusionOpenSections : exclusionOpenSections;
  const setCurrentSections =
    activeTab === "Inclusion" ? setInclusionSections : setExclusionSections;
  const setCurrentOpenSections =
    activeTab === "Inclusion" ? setInclusionOpenSections : setExclusionOpenSections;
  ...
  React.useEffect(() => {
    ...
    const loadFilterInfo = async () => {
      setIsFetching(true);
      try {
        const response = await getDrdGroupManageFilterInfo({ task_id: taskId });
        if (cancelled) return;

        const hydration = mapDrdInfoApisToDefaultSettingHydration({
          taskId,
          patientInfo: null,
          groupManageFilterInfo: response.data,
          subgroups: [],
          medicalHistoryInfo: null,
        });

        const nextFilterData = hydration.filterData ?? {
          inclusion: [],
          exclusion: [],
        };
        const nextInclusion =
          nextFilterData.inclusion.length > 0 ? nextFilterData.inclusion : [makeEmptySection(1)];
        const nextExclusion =
          nextFilterData.exclusion.length > 0 ? nextFilterData.exclusion : [makeEmptySection(1)];

        setInclusionSections(nextInclusion);
        setExclusionSections(nextExclusion);
        setInclusionOpenSections(
          Object.fromEntries(nextInclusion.map((section) => [section.id, true]))
        );
        setExclusionOpenSections(
          Object.fromEntries(nextExclusion.map((section) => [section.id, true]))
        );
        setFilterData(nextFilterData);
        setCompleted("filter", Boolean(hydration.completedItems?.filter));
      } catch (error) {
        if (cancelled) return;
        setError(error instanceof Error ? error.message : "Filter 조회에 실패했습니다.");
      } finally {
        if (!cancelled) {
          setIsFetching(false);
        }
  ...
  }, [searchParams, simulationTaskId, hydratedTaskId, setCompleted, setError, setFilterData]);

  const handleConfirm = async (): Promise<void> => {
    if (!isConfirmEnabled || isSaving) return;

    const taskId = resolveTaskId();
    if (!taskId) {
      setError("task_id를 확인할 수 없어 Filter 저장을 진행할 수 없습니다.");
      return;
    }
    ...
    try {
      ...
      const conditionRawJson = serializeDrdFilterConditionRawJson({
        inclusion: inclusionSections,
        exclusion: exclusionSections,
      });

      const payload = {
        task_id: taskId,
        condition_raw_json: conditionRawJson,
        condition_raw_version: 1,
      };

      await saveDrdGroupManageFilter(payload);
      ...
      setError(error instanceof Error ? error.message : "Filter 저장에 실패했습니다.");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const fillTestData = () => {
    ...
    setExclusionSections([{ id: 1, name: "Section 1", feature: "BMI", op: "<", value: "34" }]);
    ...
    <AppLayout headerType="drd" drdStep={1} scaleMode="none">
      ...
      >
        ...
        >
          ...
          >
            <div
              className="flex h-[40px] shrink-0 items-center justify-between px-[8px]"
            ...
            <div className="flex min-h-0 flex-1 flex-col gap-[12px]">
              <div className="flex min-h-0 flex-1 flex-row gap-[12px]">
                {/* {피처 리스트/Feature List} */}
                {/* Feature List (왼쪽 컬럼) */}
                <div className="flex w-[272px] shrink-0 flex-col gap-[12px]">
                  <div className="px-[4px]">
                    <span className="font-['Inter'] text-[19.5px] leading-[1.2] font-medium tracking-[-0.585px] text-[#484646]">
                      Feature List
                    </span>
                  </div>

                  {/* 리스트 아코디언 + 검색 필드 */}
                  <div className="flex flex-1 flex-col overflow-y-auto rounded-[24px] bg-white font-['Inter']">
                    {/* 검색 필드 */}
                    <div ref={searchContainerRef} className="relative shrink-0">
                      <div
                        className="flex h-[48px] items-center gap-[8px] bg-white px-[18px]"
                            ...
                            {Object.keys(groupedSearchResults).length === 0 ? (
                              ...
                              <>
                                {Object.entries(groupedSearchResults).map(
                                  ([category, features], catIdx, arr) => (
                                    <div
                                      key={category}
                                      style={{
                                        borderBottom:
                                          catIdx < arr.length - 1 ? "1px solid #EBEBEB" : "none",
                                      }}
                                    >
                                      {/* 카테고리 헤더 */}
                                      <div
                                        style={{ paddingTop: 4, paddingLeft: 8, paddingRight: 8 }}
                                      >
                                        <span
                                          style={{
                                            fontFamily: "Inter",
                      ...
                      return (
                        ...
                        >
                          <div
                            ...
                            className={`flex h-[48px] cursor-pointer items-center gap-[10px] px-[18px] select-none ${isActive ? "bg-[#262255]" : "hover:bg-[#f9f8fc]"}`}
                          >
                            ...
                            <span
                              className={`text-[15px] leading-none font-semibold tracking-[-0.45px] ${isActive ? "text-white" : "text-[#484646]"}`}
                            >
                              {cat.name}
                            </span>
                          </div>
                              ...
                              {infoFeatures.map((f) => {
                                ...
                                return (
                                  <div
                                    ...
                                    className={`flex h-[44px] cursor-pointer items-center border-t border-[#c7c5c9] pl-[44px] text-[15px] font-medium tracking-[-0.45px] transition-colors select-none first:border-none ${
                                      isFeatureActive
                                        ? "bg-[#efeff4] font-semibold text-[#262255]"
                                        : isFeatureHovered
                ...
                {/* 메인 설정 영역 (오른쪽 컬럼) */}
                <div className="flex flex-1 flex-col gap-[12px] overflow-y-auto rounded-[24px] bg-[rgba(255,255,255,0.6)] p-[12px]">
                  {/* 상단 탭 + 액션 버튼 */}
                  <div className="flex shrink-0 items-center justify-between">
                    {/* Inclusion / Exclusion 탭 */}
                    <div className="flex rounded-[22px] bg-white p-[4px]">
                      <button
                        onClick={() => {
                          setActiveTab("Inclusion");
                          setCheckedRows({});
                        }}
                        className={`h-[36px] cursor-pointer rounded-[36px] border-none px-[18px] text-[15px] font-semibold transition-all ${activeTab === "Inclusion" ? "bg-[#262255] text-white" : "bg-transparent text-[#484646]"}`}
                      >
                        Inclusion
                      </button>
                      <button
                        ...
                        className={`h-[36px] cursor-pointer rounded-[36px] border-none px-[18px] text-[15px] font-semibold transition-all ${activeTab === "Exclusion" ? "bg-[#262255] text-white" : "bg-transparent text-[#484646]"}`}
                      >
                        Exclusion
                      </button>
                    </div>
                    ...
                    <div className="flex items-center gap-[12px]">
                      <div className="flex gap-[4px]">
                        ...
                        <div
                          className="relative flex size-[48px] cursor-pointer items-center justify-center"
                          style={{ flexShrink: 0 }}
                        >
                          <div
                            style={{
                      ...
                      {/* Add Section 버튼 */}
                      <div
                        className="relative flex h-[48px] cursor-pointer items-center gap-[6px] rounded-[100px] px-[20px]"
                        style={{ flexShrink: 0 }}
                        onClick={addSection}
                      >
                        <div
                        ...
                        />
                        <span className="relative z-10 font-['Inter'] text-[15px] font-semibold tracking-[-0.51px] text-[#262255]">
                          Add Section
                        </span>
                        <div className="relative z-10">
                  ...
                  {/* 섹션 리스트 */}
                  <div className="scrollbar-hide flex flex-1 flex-col gap-[14px] overflow-y-auto">
                    {currentSections.map((section: Section) => {
                      const isSectionOpen = currentOpenSections[section.id];
                      return (
                        <div key={section.id} className="flex flex-col rounded-[12px] bg-white">
                          {/* 섹션 헤더 */}
                          <div
                            className={`flex h-[46px] cursor-pointer items-center gap-[10px] rounded-[12px] px-[21px] transition-colors select-none hover:bg-[#fdfdfd] ${isSectionOpen ? "rounded-b-none" : ""}`}
                          ...
                          >
                            ...
                            <span className="font-['Inter'] text-[15px] font-semibold tracking-[-0.45px] text-[#313030]">
                              {section.name}
                            </span>
                          </div>
                          ...
                          {isSectionOpen && (
                            <>
                              {/* 섹션 본문 - 첫 번째 줄 */}
                              <div className="flex h-[50px] items-center gap-[14px] px-[24px]">
                                <CustomCheckbox
                                ...
                                <div
                                  className="flex w-[16px] shrink-0 cursor-pointer items-center justify-center"
                              ...
                              {(section.subRows ?? []).map((row: SubRow, rIdx: number) => (
                                <div
                                  key={rIdx}
                                  className="flex h-[50px] items-center gap-[14px] px-[24px]"
                  ...
                  {(() => {
                    ...
                    const extractLines = (sections: Section[]) => {
                      ...
                      sections.forEach((section) => {
                        ...
                        if (mainHasData) {
                          const parts = [section.feature, section.op, section.value]
                            .filter(Boolean)
                            .join(" ");
                          lines.push({
                            key: `${section.id}-main`,
                            logic: null,
                            text: `{ ${parts} }`,
                            highlighted: Boolean(checkedRows[`${section.id}-main`]),
                          });
                        }
                        (section.subRows ?? []).forEach((row, rIdx) => {
                          const rowHasData = row.feature || row.op || row.value;
                          if (rowHasData) {
                            const parts = [row.feature, row.op, row.value]
                              .filter(Boolean)
                              .join(" ");
                            lines.push({
                              key: `${section.id}-sub-${rIdx}`,
                              logic: row.logic || "And",
                              text: `{ ${parts} }`,
                              highlighted: Boolean(checkedRows[`${section.id}-sub-${rIdx}`]),
                            });
                          }
                        });
                      });
                    ...
                    const renderLines = (lines: FormulaLine[], label: string) => (
                      <div className="flex flex-col gap-[4px] font-['Inter'] text-[15px] leading-[1.1] font-medium tracking-[-0.45px] text-[#929090]">
                        {lines.map((line, i) => (
                          <p key={line.key} className="m-0">
                            {i === 0 ? (
                              <>
                                <span className="font-semibold text-[#262255]">{label}</span> [
                                &nbsp;
                                <span
                                  style={{
                                    display: "inline-block",
                                    padding: line.highlighted ? "2px 6px" : "0",
                                    borderRadius: line.highlighted ? 8 : 0,
                                    background: line.highlighted ? "#efeff4" : "transparent",
                                    color: line.highlighted ? "#262255" : "#929090",
                                    fontWeight: line.highlighted ? 700 : 500,
                              ...
                              <>
                                ...
                                >
                                  <span
                                    className="font-bold"
                                    style={{ color: line.highlighted ? "#262255" : "#3a11d8" }}
                                  >
                                    {line.logic}
                                  </span>{" "}
                                  <span
                    ...
                    return (
                      <div
                        className="min-h-[104px] shrink-0 rounded-[12px] bg-white p-[16px]"
              ...
              <div className="flex shrink-0 justify-end gap-[12px]">
                ...
                <button
                  ...
                  className="flex h-[40px] items-center justify-center rounded-[36px] border-none px-[24px]"
                  style={{
