...
import {
  ...
  saveDrdSimulationCondition,
  type DRDSimulationScenario,
  type DiseaseDrugItem,
  type DiseaseFeatureCategoryMap,
  type DiseaseFeatureItem,
...
type DrugRowWithId = DrugRow & {
  id: number;
  pillPositions?: Array<{ leftPct: number; widthPct: number } | null>;
};

type FeatureValueOption = {
  label: string;
  columnName: string;
  disabled?: boolean;
};

type FeatureDetailOption = {
  name: string;
  values: FeatureValueOption[];
};

type FeatureCategoryOption = {
  name: string;
  details: FeatureDetailOption[];
};

const FALLBACK_FEATURE_CATEGORIES: FeatureCategoryOption[] = [
  {
    name: "Patient Info",
    details: [
      {
        ...
        values: [
          { label: "BMI", columnName: "BMI" },
          { label: "SBP", columnName: "SBP" },
        ],
      },
      {
        name: "Baseline",
        values: [
          { label: "HbA1c", columnName: "HbA1c" },
          { label: "Fasting glucose", columnName: "Fasting glucose" },
          { label: "eGFR", columnName: "eGFR", disabled: true },
        ],
      },
    ],
  },
...
const getDrugRowKey = (drug: Pick<DrugRow, "name" | "code">): string =>
  `${drug.name.trim().toLowerCase()}::${drug.code.trim().toLowerCase()}`;

const toFeatureCategories = (
  featureMap: DiseaseFeatureCategoryMap | null | undefined
): FeatureCategoryOption[] => {
  if (!featureMap || typeof featureMap !== "object") return [];

  return Object.entries(featureMap).map(([majorCategory, rawSubCategory]) => {
    const details = Object.entries(rawSubCategory ?? {}).map(([subCategory, items]) => {
      const values = (items ?? []).map((item: DiseaseFeatureItem) => ({
        label: item.item_name?.trim() || item.column_name?.trim() || `Feature ${item.id}`,
        columnName: item.column_name?.trim() || "",
      }));
...
const toDrugRows = (results: DiseaseDrugItem[]): DrugRow[] => {
  const deduped = new Map<string, DrugRow>();

  results.forEach((item) => {
    const name = item.drug_name?.trim();
    if (!name) return;

    const code = item.atc_code?.trim() || item.column_name?.trim() || `drug-${item.id}`;
    const type = item.atc_lv5_description?.trim() || "Unclassified";
    const key = getDrugRowKey({ name, code });
...
const TREND_LABELS = ["Increase", "Stable", "Decrease"] as const;

const STRATEGY_META = [
  { scenarioId: "strategy_a", strategy: "Strategy A", color: "#3a11d8", trendIndex: 0 },
  { scenarioId: "strategy_b", strategy: "Strategy B", color: "#f06600", trendIndex: 1 },
  { scenarioId: "strategy_c", strategy: "Strategy C", color: "#24c6c9", trendIndex: 2 },
] as const;
...
function ResizablePill({ disabled, name, code, testLeftPct, testWidthPct, resetKey }: { disabled?: boolean; name: string; code: string; testLeftPct?: number; testWidthPct?: number; resetKey?: number }) {
  const minW = 60;
  const timelineTotalMonths = 24;
  const timelineStepMonths = 3;
  const timelineStepCount = timelineTotalMonths / timelineStepMonths;
  ...
  useEffect(() => {
    ...
    if (!parent) return;
    const snapToGrid = (value: number, stepPx: number): number => {
      if (stepPx <= 0) return value;
      return Math.round(value / stepPx) * stepPx;
    };
    const ceilToGrid = (value: number, stepPx: number): number => {
      if (stepPx <= 0) return value;
      return Math.ceil(value / stepPx) * stepPx;
    };
    const update = () => {
      const containerW = parent.clientWidth;
      if (containerW > 0) {
        const stepPx = containerW / timelineStepCount;
        const minSnapW = stepPx;
        if (testLeftPct !== undefined && testWidthPct !== undefined) {
          const rawLeft = containerW * testLeftPct / 100;
          const rawWidth = containerW * testWidthPct / 100;
          const snappedWidth = Math.max(minSnapW, Math.min(containerW, snapToGrid(rawWidth, stepPx)));
          const snappedLeft = Math.max(0, Math.min(containerW - snappedWidth, snapToGrid(rawLeft, stepPx)));
          setLeft(snappedLeft);
          setWidth(snappedWidth);
        } else {
          const desiredWidth = Math.min(containerW, Math.max(minW, defaultW));
          const snappedWidth = Math.max(minSnapW, Math.min(containerW, ceilToGrid(desiredWidth, stepPx)));
          const snappedLeft = Math.max(0, Math.min(containerW - snappedWidth, snapToGrid(left, stepPx)));
          setLeft(snappedLeft);
          setWidth(snappedWidth);
        }
      }
  ...
  const startDrag = (type: "left" | "right" | "move") => (e: React.MouseEvent) => {
    ...
    const onMove = (mv: MouseEvent) => {
      ...
      if (dragRef.current.type === "right") {
        ...
        const clampedRawW = Math.min(
          containerW - dragRef.current.startL,
          Math.max(minSnapW, rawW)
        );
        const snappedW = Math.max(
          minSnapW,
          Math.min(containerW - dragRef.current.startL, snapToGrid(clampedRawW))
        );
        setWidth(snappedW);
      } else if (dragRef.current.type === "left") {
        const rightEdge = dragRef.current.startL + dragRef.current.startW;
        const rawL = dragRef.current.startL + dx;
        const clampedRawL = Math.max(0, Math.min(rightEdge - minSnapW, rawL));
        const snappedL = Math.max(0, Math.min(rightEdge - minSnapW, snapToGrid(clampedRawL)));
        const snappedW = Math.max(minSnapW, Math.min(containerW - snappedL, rightEdge - snappedL));
        setLeft(snappedL);
        setWidth(snappedW);
      } else {
        const rawL = dragRef.current.startL + dx;
        const clampedRawL = Math.max(
          0,
          Math.min(containerW - dragRef.current.startW, rawL)
        );
        const snappedL = Math.max(
          0,
          Math.min(containerW - dragRef.current.startW, snapToGrid(clampedRawL))
        );
        setLeft(snappedL);
      }
    };
 ...
 */
function SimulationConditionPageContent() {
  ...
  const queryDiseaseId = searchParams.get("disease_id");
  ...
  const resolveDiseaseId = (): number | null => {
    const diseaseId = Number(queryDiseaseId);
  ...
  const [featureCategories, setFeatureCategories] = useState<FeatureCategoryOption[]>(
    FALLBACK_FEATURE_CATEGORIES
  );
  const [allDrugRows, setAllDrugRows] = useState<DrugRow[]>([]);
  const [drugList, setDrugList] = useState<DrugRowWithId[]>([]);
  const nextIdRef = useRef(0); // 약물 추가 시 고유 id 생성용 카운터

  const selectedCategoryOption =
    selectedCategory !== null ? featureCategories[selectedCategory] ?? null : null;
  const detailOptions = selectedCategoryOption?.details ?? [];
  const selectedDetailOption =
    selectedDetail !== null ? detailOptions[selectedDetail] ?? null : null;
  const valueOptions = selectedDetailOption?.values ?? [];
  const selectedValueOption =
    selectedValue !== null ? valueOptions[selectedValue] ?? null : null;
  const selectedOutcomeLabel = selectedValueOption?.label ?? "[Outcome]";

  useEffect(() => {
    const diseaseId = resolveDiseaseId();
    if (!diseaseId) {
      setApiLoadError("disease_id를 확인할 수 없어 Simulation Condition API 조회를 진행할 수 없습니다.");
      return;
    }
    ...
    const loadMasterData = async () => {
      ...
      try {
        const [featureResponse, drugResponse] = await Promise.all([
          getDiseaseFeatureList({ disease_id: diseaseId }),
          getDiseaseDrugList({ disease_id: diseaseId }),
        ]);

        if (cancelled) return;

        const mappedFeatureCategories = toFeatureCategories(featureResponse.data?.features);
        setFeatureCategories(
          mappedFeatureCategories.length > 0
            ? mappedFeatureCategories
            : FALLBACK_FEATURE_CATEGORIES
        );

        const mappedDrugRows = toDrugRows(drugResponse.data?.results ?? []);
        setAllDrugRows(mappedDrugRows);
        nextIdRef.current = Math.max(nextIdRef.current, mappedDrugRows.length);
      } catch (error) {
        if (cancelled) return;
        ...
        setApiLoadError(
          error instanceof Error
            ? error.message
            : "Simulation Condition 마스터 데이터 조회에 실패했습니다."
        );
        setFeatureCategories(FALLBACK_FEATURE_CATEGORIES);
        setAllDrugRows([]);
      } finally {
        if (!cancelled) {
  ...
  useEffect(() => {
    ...
    if (!featureCategories[selectedCategory]) {
  ...
  useEffect(() => {
    if (selectedCategory === null || selectedDetail === null) return;
    if (!detailOptions[selectedDetail]) {
      setSelectedDetail(null);
      setSelectedValue(null);
    }
  }, [detailOptions, selectedCategory, selectedDetail]);

  useEffect(() => {
    if (selectedCategory === null || selectedDetail === null || selectedValue === null) return;
    if (!valueOptions[selectedValue]) {
      setSelectedValue(null);
    }
  }, [selectedCategory, selectedDetail, selectedValue, valueOptions]);
   ...
   *   - drugList: API에서 조회된 약물 상위 3개 + 전략별 pill 위치
   *   - pillResetKey 증가 → 모든 pill의 위치/크기 재계산
   */
  const handleTestLoad = () => {
    const categoryIndex = featureCategories.findIndex(
      (category) => category.details.length > 0
    );
    const nextCategory = categoryIndex >= 0 ? categoryIndex : null;
    const details = nextCategory !== null ? featureCategories[nextCategory]?.details ?? [] : [];
    const baselineIndex = details.findIndex((detail) =>
      detail.name.toLowerCase().includes("baseline")
    );
    const nextDetail =
      details.length > 0 ? (baselineIndex >= 0 ? baselineIndex : 0) : null;
    const values =
      nextDetail !== null && nextCategory !== null
        ? featureCategories[nextCategory]?.details[nextDetail]?.values ?? []
        : [];
    const hba1cIndex = values.findIndex((value) =>
      `${value.label} ${value.columnName}`.toLowerCase().includes("hba1c")
    );
    const nextValue = values.length > 0 ? (hba1cIndex >= 0 ? hba1cIndex : 0) : null;
    ...
    const sourceRows = allDrugRows;
    const presets = [
      {
        row: sourceRows[0],
        checks: [true, true, true],
        pillPositions: [{ leftPct: 0, widthPct: 100 }, { leftPct: 0, widthPct: 100 }, { leftPct: 0, widthPct: 100 }],
      },
      {
        row: sourceRows[1],
        checks: [true, true, true],
        pillPositions: [{ leftPct: 0, widthPct: 50 }, { leftPct: 0, widthPct: 100 }, { leftPct: 0, widthPct: 100 }],
      },
      {
        row: sourceRows[2],
        checks: [true, true, false],
        pillPositions: [{ leftPct: 62.5, widthPct: 37.5 }, { leftPct: 0, widthPct: 100 }, null],
      },
    ].filter(
      (
        preset
    ...
    setDrugList(
      presets.map((preset, index) => ({
        ...
        id: baseId + index,
        pillPositions: preset.pillPositions,
      }))
    );
    nextIdRef.current = baseId + presets.length;
    setPillResetKey(k => k + 1);
  };

  const maxMonths = 24;
  const monthInputMax = maxMonths;
  /** 슬라이더 thumb의 % 위치 (min=3, max=24 기준 0~100%) */
  const sliderPct = ((followUpMonths - 3) / (maxMonths - 3)) * 100;

  const clampMonthValue = (raw: string): string => {
    const trimmed = raw.trim();
    if (!trimmed) return "";
    const parsed = Number(trimmed);
    if (!Number.isFinite(parsed)) return "";
    const rounded = Math.round(parsed);
    return String(Math.min(monthInputMax, Math.max(0, rounded)));
  };

  const getStrategyTimelineMaxMonths = (index: number): number => {
    const parsed = Number(monthValues[index] ?? "");
    if (!Number.isFinite(parsed) || parsed <= 0) return monthInputMax;
    return Math.min(monthInputMax, Math.max(3, Math.round(parsed)));
  };

  const strategyMonthLabel = (index: number): string => {
    return String(getStrategyTimelineMaxMonths(index));
  };
  ...
  }, [allDrugRows, drugSearchQuery]);

  const selectedDrugKeySet = useMemo(
    () => new Set(drugList.map((drug) => getDrugRowKey(drug))),
    [drugList]
  );
  ...
  const categoryItems: CategoryItem[] = featureCategories.map((category, index) => ({
    label: category.name,
    selected: selectedCategory === index,
    saved: selectedCategory !== index,
    disabled: category.details.length === 0,
  }));
  const detailItems: CategoryItem[] = detailOptions.map((detail, index) => ({
    label: detail.name,
    selected: selectedDetail === index,
    disabled: detail.values.length === 0,
  }));
  const valueItems: CategoryItem[] = valueOptions.map((value, index) => ({
    label: value.label,
  ...
  const toPositiveMonth = (value: string, fallback: number): number => {
    const parsed = Number(value);
    if (!Number.isFinite(parsed) || parsed <= 0) {
      return Math.min(monthInputMax, Math.max(1, Math.round(fallback)));
    }
    return Math.min(monthInputMax, Math.round(parsed));
  };

  const toFiniteNumber = (value: string, fallback = 0): number => {
    const parsed = Number(value);
  ...
  const handleConfirm = async (): Promise<void> => {
    ...
    const taskId = resolveTaskId();
    if (!taskId) {
      setSaveError("task_id를 확인할 수 없어 Simulation Condition 저장을 진행할 수 없습니다.");
      return;
    }

    const targetFeature = selectedValueOption?.label?.trim() || selectedOutcomeLabel.trim();

    const trendRows = TREND_LABELS.map((label, index) => ({
      label,
      delta: inputValues[index] ?? "",
      unit: unitValues[index] ?? "%",
      month: toPositiveMonth(monthValues[index] ?? "", followUpMonths),
    }));

    const strategyScenarios: DRDSimulationScenario[] = STRATEGY_META.map((meta) => {
      const row = trendRows[meta.trendIndex];
      const dose = toFiniteNumber(row.delta);
      const interventions = drugList
        .filter((drug) => Boolean(drug.checks[meta.trendIndex]))
        .flatMap((drug) => {
          const drugName = drug.code.trim() || drug.name.trim();
          if (!drugName) {
            return [];
          }
      ...
      return {
        ...
        scenario_version: "1",
        description: `${meta.strategy} ${row.label.toLowerCase()} (${row.month}m)`,
        interventions,
      };
    });

    const feSnapshot = {
      selected_category: selectedCategoryOption?.name ?? null,
      selected_detail: selectedDetailOption?.name ?? null,
      selected_value: selectedValueOption?.label ?? null,
      target_feature: targetFeature,
      follow_up_months: followUpMonths,
      trend_rows: trendRows,
      strategy_drug_plan: strategyScenarios.map((scenario, strategyIndex) => {
        const row = trendRows[strategyIndex];
        const dose = toFiniteNumber(row?.delta ?? "");

        return {
          strategy: STRATEGY_META[strategyIndex].strategy,
          scenario_id: scenario.scenario_id,
          description: scenario.description,
          drugs: drugList
            .filter((drug) => Boolean(drug.checks[strategyIndex]))
            .map((drug) => ({
              code: drug.code,
              name: drug.name,
              dose,
    ...
    try {
      ...
      await saveDrdSimulationCondition({
        task_id: taskId,
        fe_snapshot: feSnapshot,
        ml_payload: {
          meta: {
            global_seed: DRD_MOCK_UP_DATA.global_seed,
            profile_name: DRD_MOCK_UP_DATA.profile_name,
            schema_version: DRD_MOCK_UP_DATA.schema_version,
            generator_version: DRD_MOCK_UP_DATA.generator_version,
          },
      ...
      setSimCondData({
        ...
        selectedDetail,
        selectedValue,
        selectedValueLabel: selectedValueOption?.label ?? undefined,
        followUpMonths,
        inputValues,
        unitValues,
        monthValues,
      ...
      setSaveError(
        error instanceof Error
          ? error.message
          : "Simulation Condition 저장에 실패했습니다."
      );
    } finally {
      setIsSaving(false);
    }
  ...
  return (
    <AppLayout headerType="drd" drdStep={2} scaleMode="none">
      <Loading isLoading={isPageFetching || isSaving} />
      ...
      {openUnitIdx !== null && unitDropdownPos && (
        <>
         ...
         <div style={{ display: "flex", flexDirection: "column", width: "calc(100% - 24px)", height: "100%", gap: 24, overflowX: "hidden", overflowY: "auto", marginLeft: "8px", marginRight: "8px", paddingRight: "4px" }}>
        {/* 타이틀 */}
        <div style={{ flexShrink: 0, padding: "0 12px" }}>
          ...
          {apiLoadError && (
            <div style={{ marginTop: 8, fontFamily: "Inter", fontSize: 13, fontWeight: 500, color: "#b42318", letterSpacing: "-0.36px", lineHeight: 1.3 }}>
              {apiLoadError}
            </div>
          )}
          {saveError && (
          ...
          >
            <div className="flex-1 rounded-[24px] bg-[rgba(255,255,255,0.6)] flex flex-col p-[10px] gap-[8px] overflow-y-auto">
              ...
              >
                <div className="flex items-center gap-[18px]">
                  <div className="shrink-0 flex items-center justify-center">
                    <img src={simSmilesCompleted ? "/icons/simulation-setting/state%3Dcompleted%2C%20step%3Dsmiles%2C%20Select%3DDefault%2C%20Size%3D24px.svg" : "/icons/simulation-setting/state%3Dnot%20started%2C%20step%3Dsmiles%2C%20Select%3DDefault%2C%20Size%3D24px.svg"} width={24} height={24} alt="" style={{ flexShrink: 0 }} />
                  </div>
              ...
              >
                <div className="flex items-center gap-[18px]">
                  <div className="shrink-0 flex items-center justify-center">
                    <img src="/icons/simulation-setting/state%3Dcompleted%2C%20step%3Dsimulation%2C%20Select%3DDefault%2C%20Size%3D24px.svg" width={24} height={24} alt="" style={{ flexShrink: 0 }} />
                  </div>
          ...
          {/* 오른쪽 상위 배경 카드: selection-bg.png → 안에 흰색 테이블 카드 */}
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 12, position: "relative", zIndex: 1, minHeight: 0, overflow: "auto", minWidth: 600 }}>
              ...
              <div style={{ background: "rgba(255,255,255,0.60)", borderRadius: 24, paddingTop: 16, paddingBottom: 16, paddingLeft: 16, paddingRight: 16, display: "flex", flexDirection: "column", gap: 12, flex: "1 1 0", minHeight: 0, overflow: "visible", position: "relative", zIndex: 1 }}>
                ...
                <div style={{ display: "flex", gap: 17, flex: 1, minHeight: 0, position: "relative", zIndex: 2 }}>
                  ...
                  />
                  <div style={{ flex: 1, minWidth: 0, opacity: selectedCategory === null || detailItems.length === 0 ? 0.4 : 1, pointerEvents: selectedCategory === null || detailItems.length === 0 ? "none" : "auto", transition: "opacity 0.2s" }}>
                    <CategoryFrame
                      header="Detail"
                      items={detailItems}
                      onSelect={(i) => {
                  ...
                  <div style={{ flex: 1, minWidth: 0, opacity: selectedDetail === null || valueItems.length === 0 ? 0.4 : 1, pointerEvents: selectedDetail === null || valueItems.length === 0 ? "none" : "auto", transition: "opacity 0.2s" }}>
                    <CategoryFrame header="Value" items={valueItems} onSelect={setSelectedValue} />
                  </div>
                </div>
                          ...
                          <div style={{ display: "flex", gap: 8, flex: 1 }}>
                            ...
                            <div style={{ position: "relative", flex: 1 }}>
                              <div onClick={(e) => { if (openUnitIdx === i) { setOpenUnitIdx(null); setUnitDropdownPos(null); } else { const rect = (e.currentTarget as HTMLElement).getBoundingClientRect(); setUnitDropdownPos({ top: rect.bottom + 2, left: rect.left, width: rect.width }); setOpenUnitIdx(i); } }} style={{ background: "#efeff4", borderRadius: 8, height: 36, display: "flex", alignItems: "center", paddingLeft: 12, paddingRight: 4, cursor: "pointer", gap: 2 }}>
                                ...
                                <img src="/icons/disclosure/Property%201%3DClose%2C%20Size%3D18.svg" width={18} height={18} alt="" style={{ flexShrink: 0 }} />
                              </div>
                            </div>
                            <input type="text" value={monthValues[i]} onChange={(e) => setMonthValues(prev => prev.map((v, idx) => idx === i ? clampMonthValue(e.target.value) : v))} className="placeholder:text-[#c6c5c9]" style={{ flex: 1, minWidth: 0, height: 36, background: "#efeff4", borderRadius: 8, border: "none", paddingLeft: 12, paddingRight: 12, fontFamily: "Inter", fontWeight: 500, fontSize: 17, color: "#484646", letterSpacing: "-0.68px", outline: "none", cursor: "text" }} />
                          </div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div ref={followUpRef} style={{ position: "relative" }}>
                      <div onClick={() => setFollowUpOpen((v) => !v)} style={{ background: "#efeff4", borderRadius: 8, height: 28, minWidth: 52, display: "flex", alignItems: "center", paddingLeft: 8, paddingRight: 4, gap: 2, cursor: "pointer", userSelect: "none" }}>
                        ...
                        <div style={{ width: 18, height: 18, display: "flex", alignItems: "center", justifyContent: "center", transform: followUpOpen ? "rotate(180deg)" : "rotate(0deg)", transition: "transform 0.15s" }}>
                          <img src="/icons/disclosure/Property%201%3DClose%2C%20Size%3D18.svg" width={18} height={18} alt="" />
                        </div>
                      </div>
                      {followUpOpen && (
                        <>
                ...
                <div style={{ display: "flex", gap: 17, flex: 1, minHeight: 0, overflow: "hidden" }}>
                          ...
                          >
                            {drugSearchResults.length === 0 ? (
                              ...
                              drugSearchResults.map((drug, i) => {
                                const isAlreadySelected = selectedDrugKeySet.has(getDrugRowKey(drug));
                                ...
                                return (
                                  <div
                                    ...
                                    onMouseDown={() => {
                                      ...
                                      setDrugList((prev) => {
                                        const nextKey = getDrugRowKey(drug);
                                        if (prev.some((selectedDrug) => getDrugRowKey(selectedDrug) === nextKey)) {
                                          return prev;
                                        }

                                        const newId = nextIdRef.current++;
                                        const initialPillPositions = [0, 1, 2].map((strategyIdx) => ({
                                          leftPct: 0,
                                          widthPct: (getStrategyTimelineMaxMonths(strategyIdx) / monthInputMax) * 100,
                                        }));
                                        return [
                                          ...prev,
                                          {
                                            ...drug,
                                            id: newId,
                                            checks: [true, true, true] as boolean[],
                                            pillPositions: initialPillPositions,
                                          },
                                        ];
                                      });
                                      setDrugSearchQuery("");
                                  ...
                                  >
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                      <div style={{ fontFamily: "Inter", fontSize: 9, fontWeight: 500, color: "#aaaaad", letterSpacing: "-0.27px", lineHeight: 1.05 }}>
                                        {drug.type}
                                      </div>
                                    ...
                                    <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
                                      {isAlreadySelected && (
                                        <span style={{ fontFamily: "Inter", fontSize: 9, fontWeight: 600, color: "#3a11d8", letterSpacing: "-0.27px" }}>
                                          Selected
                                        </span>
                                      )}
                                      <span style={{ fontFamily: "Inter", fontSize: 9, fontWeight: 500, color: "#aaaaad", letterSpacing: "-0.27px" }}>
                                        {drug.code}
                                      </span>
                                    </div>
                                  </div>
                  ...
                  <div style={{ flex: "2.5 2.5 0", minWidth: 0, overflow: "hidden", display: "flex", flexDirection: "column", gap: 0 }}>
                    ...
                    <div style={{ background: "white", borderRadius: 16, overflow: "auto", flex: 1, display: "flex", flexDirection: "column" }}>
                      {([
                        { strategy: "Strategy A", color: "#3a11d8", trend: "Increase", strategyIdx: 0 },
                        { strategy: "Strategy B", color: "#f06600", trend: "Stable", strategyIdx: 1 },
                        { strategy: "Strategy C", color: "#24c6c9", trend: "Decrease", strategyIdx: 2 },
                      ]).map((row, ri) => (
                        <div key={ri} style={{ flex: 1, minHeight: 80, display: "flex", borderTop: ri > 0 ? "1px solid #e2e1e5" : undefined }}>
                          ...
                          <div style={{ width: "clamp(140px, 22%, 236px)", flexShrink: 0, borderRight: "1px solid #e2e1e5", display: "flex", flexDirection: "column", gap: 6, paddingTop: 2, paddingBottom: 2 }}>
                            ...
                            <div style={{ display: "flex", flexDirection: "column", paddingLeft: 16, paddingRight: 8 }}>
                              <span style={{ fontFamily: "Inter", fontSize: 12, fontWeight: 500, color: "#1c1b1b", letterSpacing: "-0.48px", lineHeight: 1.1 }}>
                                {selectedOutcomeLabel === "[Outcome]" ? "Outcome Trend" : `${selectedOutcomeLabel} Trend`}
                              </span>
                              <span style={{ fontFamily: "Inter", fontSize: 12, fontWeight: 500, color: "#1c1b1b", letterSpacing: "-0.48px", lineHeight: 1.1 }}>
                                {`${row.trend} : ${strategyMonthLabel(row.strategyIdx)} Months`}
                              </span>
                            </div>
                          </div>
                          ...
                          <div style={{ flex: 1, minWidth: 0, position: "relative", display: "flex", flexDirection: "column", paddingTop: 8, paddingBottom: 8, paddingLeft: 12, paddingRight: 12, gap: 4, alignItems: "stretch", justifyContent: "center" }}>
                            ...
                            {drugList.map((drug) => {
                              ...
                              const widthPct = pos ? pos.widthPct : 100;
            ...
            <div className="shrink-0 flex justify-end gap-[12px]">
              ...
              <button
                ...
                style={{ cursor: canConfirm && !isSaving ? "pointer" : "not-allowed" }}
              >
                <div className="absolute inset-0" style={{ background: canConfirm && !isSaving ? "#f06600" : "#c6c5c9", transition: "background 0.15s" }} />
                <span className="relative z-10 font-['Inter'] font-semibold text-[17px] leading-[1.05] text-white text-center tracking-[-0.51px]">
...
export default function SimulationConditionPage() {
  return (
    <Suspense fallback={null}>
      <SimulationConditionPageContent />
    </Suspense>
  );
}
