...
export default function SimulationPage() {
  ...
  const {
    filteredData,
    chartData,
    defaultPowerIndex: _defaultPowerIndex,
      ...
      reductionView: {
        ...
        charts: [
          ...
          },
          {
            ...
            optivis: Number((optivis.cost / 1000000).toFixed(1)),
            traditional: Number((traditional.cost / 1000000).toFixed(1)),
  ...
  const isResultMasking = isLoading;

  const displaySimulationData = useMemo(() => {
    if (!simulationData || !isResultMasking) {
      return simulationData;
    }

    const maskNullableValue = (value: string | null): string | null => (value === null ? null : "");

    return {
      ...simulationData,
      smallerSample: {
        ...simulationData.smallerSample,
        percentage: "",
        chartData: {
          optivis: [],
          traditional: [],
      ...
      comparisonTable: {
        ...
        sampleSize: {
          optivis: {
            treatmentGroup1: "",
            treatmentGroup2: maskNullableValue(
              simulationData.comparisonTable.sampleSize.optivis.treatmentGroup2
            ),
            treatmentGroup3: maskNullableValue(
  ...
  }, [simulationData, isResultMasking]);

  const displayChartDataToUse = useMemo(() => {
    if (!chartDataToUse || !isResultMasking) {
  ...
  }, [chartDataToUse, isResultMasking]);
      ...
      <AppLayout headerType="ats" scaleMode="height">
        ...
        >
          <div style={{ flexShrink: 0, padding: "0 12px" }}>
            <div className="text-title text-neutral-5 text-left">Trial Design Optimization</div>
            <p className="text-body2m text-left text-neutral-50">Optimize study design</p>
          </div>

          <div className="ats-simulation-layout flex min-h-0 w-full flex-1 gap-[0px]">
            <div className="ats-left-panel min-h-0 min-w-[280px] flex-[22]">
              <LeftPanel
            ...
            <div
              className="ats-right-panel min-h-0 min-w-0 flex-[78]"
            ...
            >
              <RightPanel
                ...
                isApplied={isApplied}
                simulationData={displaySimulationData}
                chartDataToUse={displayChartDataToUse}
                getHighlightXValue={getHighlightXValue}
                apiData={apiData}
                isResultMasking={isResultMasking}
              />
            </div>
          </div>
        </div>
