...
`docs/DRD_API_GUIDE_v1.1.pdf`(14p) 내용을 정리한 문서입니다.  
...
| PUT    | `/api/nexus/group-manage/{entity_type}/simulation-condition/save/`  | DRD 시뮬레이션 조건 저장            | 13         | `entity_type: drd`                     |
| PATCH  | `/api/nexus/group-manage/{entity_type}/simulation-condition/clear/` | DRD 시뮬레이션 조건 비활성화(clear) | 14         | `entity_type: drd`                     |
| GET    | `/api/nexus/group-manage/{entity_type}/simulation-condition/info/`  | DRD 시뮬레이션 조건 조회            | 14         | `entity_type: drd`                     |
...
- PDF 본문에서 줄바꿈으로 분리된 경로는 실제 URL 형태로 정규화했습니다.
  ...
  - 예: `group-` + `manage/{entity_type}/simulation-condition/...` -> `group-manage/{entity_type}/simulation-condition/...`
